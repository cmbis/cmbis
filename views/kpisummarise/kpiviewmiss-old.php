...
<?php
    ...
    $sql='SELECT count(*) FROM '.$_GET['table'].
        ' INNER JOIN cmbis_area_hosp ON cmbis_area_hosp.VID = '.$_GET['table'].'.vhid'. 
    ...
    'sql' => 'SELECT *, right(vhid,2) as mu,TIMESTAMPDIFF(YEAR,'.$_GET['table'].'.BIRTH,NOW()) as age  FROM '.$_GET['table'].
        ' INNER JOIN cmbis_area_hosp ON cmbis_area_hosp.VID = '.$_GET['table'].'.vhid'.
